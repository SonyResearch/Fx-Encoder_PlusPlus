...
def sample_fx(n_fx, probabilities_dict):
   selected = []
   available = list(probabilities_dict.keys())
   
   while len(selected) < n_fx and available:
       idx = random.randrange(len(available))
       fx = available.pop(idx)
       
       if random.random() < probabilities_dict[fx]:
           selected.append(fx)
   
   return selected

def get_loss_weights(step, steps=5000):
    if step < steps:
        global_w = 1.0 
        local_w = 0.0
        # progress = step / steps
        # global_w = 0.9 - (0.1 * progress) 
        # local_w = 1 - global_w
    else:
        global_w = 0.6
        local_w = 0.4
    return global_w, local_w

METER = Meter(44100)
# def loudness_normalize(audio, target_loudness=None):
#     global METER
#     METER = METER.to(audio.device)
#     audio = audio.permute(0, 2, 1)
#     loudness = METER.integrated_loudness(audio) + 1e-8
#     if target_loudness is None:
#         target_loudness = torch.full((loudness.shape[0],), -18, device=audio.device)
#     else:
#         target_loudness = target_loudness.to(audio.device)

#     delta_loudness = target_loudness - loudness
#     delta_loudness = torch.clamp(delta_loudness, -20.0, 20.0) 
#     gain = torch.pow(10.0, delta_loudness / 20.0).unsqueeze(-1).unsqueeze(-1)
#     audio = audio.permute(0, 2, 1)
#     return audio * gain
def loudness_normalize(audio, target_loudness=None):
    global METER
    ...
    loudness = METER.integrated_loudness(audio) + 1e-8
    if target_loudness is None:
        target_loudness = torch.full((loudness.shape[0],), -14, device=audio.device)
    else:
        target_loudness = target_loudness.to(audio.device)

    loudness = torch.where(
        torch.isinf(loudness) | (loudness < -70),  
        torch.tensor(-70.0, device=audio.device), 
        loudness
    )
    delta_loudness = target_loudness - loudness
